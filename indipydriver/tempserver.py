...
class IPyServer:
    ...
    def __init__(self, *drivers, host="localhost", port=7624, maxconnections=5):
        ...
        # all data is sent on one sig with format con_id, xmldata
        # con_id is used to stop a transmitter receiving its own signal

        self.xml_data_sig = Signal()
        ...
        for c in range(0, maxconnections):
            con_id += 1
            self.connectionpool.append(_ClientConnection(con_id, self.xml_data_sig))
        ...
        for driver in self.drivers:
            ...
            con_id += 1
            driver.comms = _DriverComms(driver, con_id, self.xml_data_sig)
...
class _DriverComms:
    ...
    def __init__(self, driver, con_id, self.xml_data_sig):

        # This object is attached to this driver
        self.driver = driver
        self.con_id = con_id
        self.xml_data_sig = xml_data_sig
        self.xml_data_sig.connect(self.run_rx)
        self.drv_rx_sig = Signal()


    async def self.run_rx(self, sender, **kw):
        "Gets data from xml_data_sig, and sends it to the driver"
        if self.con_id == kw["con_id"]:
            return
        xmldata = kw["xmldata"]
        # call the drivers receive data function
        self.driver.receivedata(xmldata)


    async def run_tx(self, sender, **kw):
        """Called by the drivers own tx signal"""
        xmldata = kw["xmldata"]
        # check for duplicate devicename ###########################
        self.xml_data_sig.send(self, con_id=self.con_id, xmldata=xmldata)
